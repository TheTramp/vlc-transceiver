...
constexpr int SERIAL_BAUD = B2000000; // 2 Mbps
constexpr uint8_t SYNC_WORD_1 = 0xAA;
constexpr uint8_t SYNC_WORD_2 = 0xD4;

/* ANSI X3T9.5 4B5B Encoding Table */
...
int main() {
    ...
    std::cerr << "[TX] 3-Minute Benchmark Transmitter Ready. Blasting data...\n";
    
    // Create the "Golden Payload" - e.g., an alternating bit pattern 0x55 (01010101)
    uint8_t golden_payload[PAYLOAD_SIZE];
    for(size_t i = 0; i < PAYLOAD_SIZE; ++i) {
        golden_payload[i] = 0x55; 
    }
    ...
    while (true) {
        ...
        raw_packet.reserve(2 + PAYLOAD_SIZE + 2); 
        
        raw_packet.push_back(static_cast<uint8_t>(PAYLOAD_SIZE));
        raw_packet.push_back(sequence++);
        raw_packet.insert(raw_packet.end(), golden_payload, golden_payload + PAYLOAD_SIZE);
